from django.urls import path

from .views import NoteAPIView, NotesDetailAPIView, DeletedNoteAPIView, DeletedNoteDetailAPIView, CreateUserView, LoginView, LogoutView, UserView

urlpatterns = [
    path('sign-up/', CreateUserView.as_view(), name='sign-up'),
    path('login/', LoginView.as_view(), name='login'),
    path('logout/', LogoutView.as_view(), name='logout'),
    path('notes/', NoteAPIView.as_view(), name='notes'),
    path('deleted-notes/', DeletedNoteAPIView.as_view(), name='deleted-notes'),
    path('restore-notes/<uuid:note_id>/', DeletedNoteDetailAPIView.as_view(), name='restore-notes'),
    path('notes/<uuid:note_id>/', NotesDetailAPIView.as_view(), name='note-detail'),
    path('user/', UserView.as_view(), name='user'),
]